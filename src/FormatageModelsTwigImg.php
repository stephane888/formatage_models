...
trait FormatageModelsTwigImg {
	 ...
	 */
	public function getLayoutValuesImg(array $build, array $keySearch = []) {
		$vals = [];
		$key = 0;
		foreach ($build as $value) {
			if (is_array($value) && ! empty($value)) {
				if (! empty($value['#theme']) && $value['#theme'] == 'block' && ! empty($value['content'])) {
					$vals[] = $this->getFieldImgValue($value['content'], $keySearch);
				}
			}
			$key ++;
		}
	...
	public function getFieldImgValue($build, $keySearch) {
